...
light_translation = Variable(torch.from_numpy(\
    np.array([-0.5,-0.5,-0.5],dtype=np.float32)), requires_grad=True)
...
img = render(0, *args)
image.imwrite(img.data.numpy(), 'test/results/test_shadow_2/target.exr')
image.imwrite(img.data.numpy(), 'test/results/test_shadow_2/target.png')
target = Variable(torch.from_numpy(image.imread('test/results/test_shadow_2/target.exr')))
shape_light.vertices=light_vertices+light_translation
args=render_pytorch.RenderFunction.serialize_scene(cam,materials,shapes,lights,resolution,256,1)
img = render(1, *args)
image.imwrite(img.data.numpy(), 'test/results/test_shadow_2/init.png')
diff = torch.abs(target - img)
image.imwrite(diff.data.numpy(), 'test/results/test_shadow_2/init_diff.png')
...
for t in range(200):
    ...
    img = render(t, *args)
    image.imwrite(img.data.numpy(), 'test/results/test_shadow_2/iter_{}.png'.format(t))
...
img = render(202, *args)
image.imwrite(img.data.numpy(), 'test/results/test_shadow_2/final.exr')
image.imwrite(img.data.numpy(), 'test/results/test_shadow_2/final.png')
image.imwrite(np.abs(target.data.numpy() - img.data.numpy()), 'test/results/test_shadow_2/final_diff.png')

from subprocess import call
call(["ffmpeg", "-framerate", "24", "-i",
    "test/results/test_shadow_2/iter_%d.png", "-vb", "20M",
    "test/results/test_shadow_2/out.mp4"])