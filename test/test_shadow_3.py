...
# img = render(0, *args)
# image.imwrite(img.data.numpy(), 'results/test_shadow_3/target.exr')
# exit()

args=render_pytorch.RenderFunction.serialize_scene(cam,materials,shapes,lights,resolution,256,1)
render = render_pytorch.RenderFunction.apply
img = render(0, *args)
image.imwrite(img.data.numpy(), 'test/results/test_shadow_3/target.exr')
image.imwrite(img.data.numpy(), 'test/results/test_shadow_3/target.png')
target = Variable(torch.from_numpy(image.imread('test/results/test_shadow_3/target.exr')))
shape_floor.vertices=Variable(torch.from_numpy(\
    np.array([[-2.0,-0.2,-2.0],[-2.0,-0.2,2.0],[2.0,-0.2,-2.0],[2.0,-0.2,2.0]],dtype=np.float32)),
    requires_grad=True)
args=render_pytorch.RenderFunction.serialize_scene(cam,materials,shapes,lights,resolution,256,1)
img = render(1, *args)
image.imwrite(img.data.numpy(), 'test/results/test_shadow_3/init.png')
diff = torch.abs(target - img)
image.imwrite(diff.data.numpy(), 'test/results/test_shadow_3/init_diff.png')

optimizer = torch.optim.Adam([shape_floor.vertices], lr=5e-3)
for t in range(200):
    print('iteration:', t)
    ...
    args=render_pytorch.RenderFunction.serialize_scene(cam,materials,shapes,lights,resolution,4,1)
    img = render(t + 1, *args)
    image.imwrite(img.data.numpy(), 'test/results/test_shadow_3/iter_{}.png'.format(t))
    ...
    optimizer.step()
    print('floor_vertices:', shape_floor.vertices)

args=render_pytorch.RenderFunction.serialize_scene(\
    cam,materials,shapes,lights,resolution,256,1)
img = render(202, *args)
image.imwrite(img.data.numpy(), 'test/results/test_shadow_3/final.exr')
image.imwrite(img.data.numpy(), 'test/results/test_shadow_3/final.png')
image.imwrite(np.abs(target.data.numpy() - img.data.numpy()), 'test/results/test_shadow_3/final_diff.png')

from subprocess import call
call(["ffmpeg", "-framerate", "24", "-i",
    "test/results/test_shadow_3/iter_%d.png", "-vb", "20M",
    "test/results/test_shadow_3/out.mp4"])