...
def gen_look_at_matrix(pos, look, up):
    ...
    left = normalize(torch.cross(normalize(up), d))
    new_up = normalize(torch.cross(d, left))
    z = Variable(torch.zeros([1], dtype=torch.float32))
    o = Variable(torch.ones([1], dtype=torch.float32))
    return torch.transpose(torch.stack([torch.cat([left  , z], 0),
                                        torch.cat([new_up, z], 0),
                                        torch.cat([d     , z], 0),
                                        torch.cat([pos   , o], 0)]), 0, 1)
...
def torch_rotate_matrix(angles):
    theta = angles[0]
    phi = angles[1]
    psi = angles[2]
    rot_x = Variable(torch.zeros(3, 3), requires_grad=False)
    rot_y = Variable(torch.zeros(3, 3), requires_grad=False)
    rot_z = Variable(torch.zeros(3, 3), requires_grad=False)
    rot_x[0, 0] = 1
    rot_x[0, 1] = 0
    rot_x[0, 2] = 0
    rot_x[1, 0] = 0
